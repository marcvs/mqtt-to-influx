...
class ParseMqttData:
    ...
    def __init__(self, msg):
        ...
        if debug:
            print ("Detected a {} message".format(self.msg_type))
            if (self.msg_type in ( None, 'tasmota', 'homegear', 'mh_sensor_v1')):
                print ("  topic[-1]:      {} ".format(msg.topic.split("/")[-1]))
                if not self.is_json:
                    print ("  topic:message: {: <38}{}".format(msg.topic, msg.payload))

                if self.is_json:
                    print ("  json")
                    print(json.dumps(self.msg_json, sort_keys=True, indent=4, separators=(',', ': ')))
